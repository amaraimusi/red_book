...
jQuery(()=> {
	...
	$('#en_sp_tbl').show();// 高速表示のためテーブルは最後に表示する
...
function init(){
	...
	// CBBXS-2023
	// 綱IDリストJSON
	let bioClsIdList = crudBaseData.masters.bioClsIdList;
	disFilData['bio_cls_id'] ={'fil_type':'select','option':{'list':bioClsIdList}};
	// 絶滅危惧種カテゴリーIDリストJSON
	let enCtgIdList = crudBaseData.masters.enCtgIdList;
	disFilData['en_ctg_id'] ={'fil_type':'select','option':{'list':enCtgIdList}};
	...
	crudBase.crudBaseBulkAdd.init(
		[
			...
			{'field':'bio_cls_id', 'inp_type':'select', 'list':bioClsIdList, 'def':0}, 
			{'field':'en_ctg_id', 'inp_type':'select', 'list':enCtgIdList, 'def':0}, 
			{'field':'endemic_sp_flg', 'inp_type':'textarea'}, 
			{'field':'sort_no', 'inp_type':'textarea'}, 
			{'field':'delete_flg', 'inp_type':'textarea'}, 

			// CBBXE
			
//			{'field':'en_sp_group', 'inp_type':'select', 'list':en_spGroupList, 'def':2}, 
//			{'field':'en_sp_date', 'inp_type':'date', 'def':today}, 
//			{'field':'note', 'inp_type':'text', 'def':'TEST'}, 
//			{'field':'sort_no', 'inp_type':'sort_no', 'def':1}, 
		],
		{
			ajax_url:'en_sp/bulk_reg',
			csrf_token:csrf_token,
			ta_placeholder:"Excelからコピーした絶滅危惧種名、絶滅危惧種数値を貼り付けてください。（タブ区切りテキスト）\n(例)\n絶滅危惧種名A\t100\n絶滅危惧種名B\t101\n",
		}
	);
...
function _getValidMethods(){
	let methods = {
			...
			kj_scien_name:(cbv, value)=>{
				let err = '';
				// 文字数バリデーション
				if(!cbv.isMaxLength(value, 225)){
					err = '225文字以内で入力してくだい。';
				}
				return err;
			},
...
function calendarViewKShow(){
	// カレンダービューを生成 
	crudBase.calendarViewCreate('en_sp_date');
}
