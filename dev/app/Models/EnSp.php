...
class EnSp extends Model
{
	protected $table = 'en_sps'; // 紐づけるテーブル名
	...
	private function createKjConditions($kjs){
		...
		if(!empty($kjs['kj_main'])){
			$cnds[]="CONCAT( IFNULL(EnSp.en_sp_name, '') ,IFNULL(EnSp.note, '')) LIKE '%{$kjs['kj_main']}%'";
		}
		...
		$kj_endemic_sp_flg = $kjs['kj_endemic_sp_flg'];
		if(!empty($kjs['kj_endemic_sp_flg']) || $kjs['kj_endemic_sp_flg'] ==='0' || $kjs['kj_endemic_sp_flg'] ===0){
			if($kjs['kj_endemic_sp_flg'] != -1){
