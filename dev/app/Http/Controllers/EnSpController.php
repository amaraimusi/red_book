...
class EnSpController
{
	...
	public function index(){
		...
		// 綱IDリスト
		$bioClsIdList = $this->md->getBioClsIdList();
		$masters['bioClsIdList'] = $bioClsIdList;

		// 絶滅危惧種カテゴリーIDリスト
		$enCtgIdList = $this->md->getEnCtgIdList();
		$masters['enCtgIdList'] = $enCtgIdList;
		...
		return view('en_sp.index', compact('data', 'crudBaseData', 'crud_base_json'));
	...
	private function init(){
		...
		$kensakuJoken=[
				...
				// CBBXS-2000
				['name'=>'kj_id', 'def'=>null],
				['name'=>'kj_bio_cls_id', 'def'=>null],
				['name'=>'kj_family_name', 'def'=>null],
				['name'=>'kj_wamei', 'def'=>null],
				['name'=>'kj_scien_name', 'def'=>null],
				['name'=>'kj_en_ctg_id', 'def'=>null],
				['name'=>'kj_endemic_sp_flg', 'def'=>null],
		...
		$model = new EnSp(); // モデルクラス
		
		$crudBaseData = [
				'fw_type' => 'laravel7',
				'model_name_c' => 'EnSp',
				'tbl_name' => 'en_sps', // テーブル名をセット
	...
	private function getDataForDownload(){
		...
		//セッションから検索条件情報を取得
		$kjs=session('en_sp_kjs');

		// セッションからページネーション情報を取得
		$pages = session('en_sp_pages');
