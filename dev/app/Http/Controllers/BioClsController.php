...
use App\Models\BioCls;
use Illuminate¥Support¥Facades¥DB;

class BioClsController
{
	...
	public function index(){
		...
		return view('bio_cls.index', compact('data', 'crudBaseData', 'crud_base_json'));
	...
	private function init(){
		...
		$model = new BioCls(); // モデルクラス
		
		$crudBaseData = [
				'fw_type' => 'laravel7',
				'model_name_c' => 'BioCls',
				'tbl_name' => 'bio_clss', // テーブル名をセット
	...
	private function getDataForDownload(){
		...
		//セッションから検索条件情報を取得
		$kjs=session('bio_cls_kjs');

		// セッションからページネーション情報を取得
		$pages = session('bio_cls_pages');
	...
	public function bulk_reg(){
		...
		$res = $bulkReg->reg('bio_clss', $param);
