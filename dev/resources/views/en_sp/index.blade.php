...
<head>
	...
	<title>絶滅危惧種管理画面</title>
	
	<script src="{{ asset('/js/app.js') }}" defer></script>
	<script src="{{ $crud_base_js . $ver_str }}" defer></script>
	<script src="{{ asset('/js/EnSp/index.js')  . $ver_str }}" defer></script>
...
@include('layouts.common_header')
	...
	<!-- 検索条件入力フォーム -->
	<form action="" class="form_kjs" id="en_spIndexForm" method="post" accept-charset="utf-8">
		
		<?php $this->CrudBase->inputKjMain('kj_main','',null,'絶滅危惧種名、備考を検索する');?>
		...
		$this->CrudBase->inputKjSelect('kj_bio_cls_id','綱ID', $masters['bioClsIdList']); 
		$this->CrudBase->inputKjText('kj_family_name','科');
		$this->CrudBase->inputKjText('kj_wamei','和名');
		$this->CrudBase->inputKjText('kj_scien_name','学名');
		$this->CrudBase->inputKjSelect('kj_en_ctg_id','絶滅危惧種カテゴリーID', $masters['enCtgIdList']); 
		$this->CrudBase->inputKjFlg('kj_endemic_sp_flg','固有種フラグ');
...
<!-- 一覧テーブル -->
<table id="en_sp_tbl" class="table table-striped table-bordered table-condensed" style="display:none;margin-bottom:0px">
...
foreach($data as $i=>&$ent){
	...
	$this->CrudBase->tdFlg($ent, 'endemic_sp_flg');
...
<table id="crud_base_forms">
	...
	<tr id="ajax_crud_new_inp_form" class="crud_base_form" style="display:none;padding-bottom:60px"><td colspan='5'>
		...
		<div class="cbf_inp_wrap">
			<div class='cbf_inp_label' >綱ID: </div>
			<div class='cbf_input'>
		...
		<div class="cbf_inp_wrap">
			<div class='cbf_inp_label' >絶滅危惧種カテゴリーID: </div>
			<div class='cbf_input'>
		...
		<div class="cbf_inp_wrap">
			<div class='cbf_inp_label' >固有種フラグ: </div>
			<div class='cbf_input'>
				<input type="checkbox" name="endemic_sp_flg" class="valid"/>
